    ...
    if ((self.maxConcurrentFileDownloadsCount == -1) || ((NSInteger)self.activeDownloadsDictionary.count < self.maxConcurrentFileDownloadsCount))
    {
        ...
        if (floor(NSFoundationVersionNumber) > NSFoundationVersionNumber_iOS_6_1)
        {
            ...
            else if (aRemoteURL)
            {
                ...
                {
                    aURLRequest = [[self.fileDownloadDelegate urlRequestForRemoteURL:aRemoteURL] mutableCopy];
                }
                else
                {
                    NSTimeInterval aRequestTimeoutInterval = 60.0; // iOS default value
                    aURLRequest = [[NSMutableURLRequest alloc] initWithURL:aRemoteURL cachePolicy:NSURLRequestReloadIgnoringLocalCacheData timeoutInterval:aRequestTimeoutInterval];
                    [aURLRequest setValue:@"application/json" forHTTPHeaderField:@"Content-Type"];
                    [aURLRequest setValue:[DataManager sharedManager].user.email  forHTTPHeaderField:@"webuser-id"];
                    [aURLRequest setValue:[DataManager sharedManager].user.sessionID  forHTTPHeaderField:@"webuser-sessionid"];
                }
                if (aURLRequest)
                {
