...
@interface BankBranchAutoComplete ()
<UITextFieldDelegate,MLPAutoCompleteTextFieldDelegate, MLPAutoCompleteTextFieldDataSource
>
{
    NSString* customString;
    NSArray* curArray;
    BankBranchModel* curModel;
    NSString* serverAPI;
    NSString* sessionID;
}
@end

@implementation BankBranchAutoComplete
...
- (void) selectModel {
    BOOL isSame = NO;
    for (BankBranchModel* model in curArray) {
    ...
    if (!isSame) {
        curModel = [BankBranchModel new];
        curModel.bankBranchCode = @"";
        curModel.name = customString;
    }
}
...
- (NSArray*) parseResponse: (id) response
{
    ...
    for (id obj in response) {
        [keywords addObject:[obj valueForKeyNotNull:@"name"]];
    }
