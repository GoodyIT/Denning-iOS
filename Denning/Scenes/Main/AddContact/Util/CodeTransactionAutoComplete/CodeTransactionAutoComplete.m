...
@implementation CodeTransactionAutoComplete
...
- (void) selectModel {
    BOOL isSame = NO;
    for (CodeTransactionDesc* model in curArray) {
        if ([model.strTransactionDescription isEqualToString:customString]) {
            curModel = model;
            isSame = YES;
            break;
        }
    ...
    if (!isSame) {
        curModel = [CodeTransactionDesc new];
        curModel.codeValue = @"";
        curModel.strTransactionDescription = customString;
    }
}
...
- (NSArray*) parseResponse: (id) response
{
    ...
    for (id obj in response) {
        [keywords addObject:[obj valueForKeyNotNull:@"strTransactionDescription"]];
    }
