...
@interface ListOfCodeStrViewController ()
...
@property (strong, nonatomic) NSMutableArray<CodeStrModel*>* listOfStrCode;
...
@implementation ListOfCodeStrViewController
...
- (void) getList {
    ...
    [[QMNetworkManager sharedManager] sendPrivateGetWithURL:_url completion:^(NSDictionary * _Nonnull result, NSError * _Nonnull error, NSURLSessionDataTask * _Nonnull task) {
        ...
        if (error == nil) {
            NSArray* array = [CodeStrModel getAddStrArrayFromResponse:(NSArray*)result];
            if (result.count != 0) {
                self.page = [NSNumber numberWithInteger:[self.page integerValue] + 1];
            }
            if (isAppending) {
                self.listOfStrCode = [[self.listOfStrCode arrayByAddingObjectsFromArray:array] mutableCopy];
            } else {
                self.listOfStrCode = [array mutableCopy];
            }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    return self.listOfStrCode.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textLabel.text = self.listOfStrCode[indexPath.row].strCity;
...
{
    _updateHandler(_listOfStrCode[indexPath.row]);
