...
- (void)viewDidLoad {
    ...
    for (NSNumber *uID in self.session.opponentsIDs) {
        ...
        if (Core.currentUser.ID == uID.integerValue) {
            ...
            QBUUser *initiator = [QMCore.instance.usersService.usersMemoryStorage userWithID:self.session.initiatorID.unsignedIntegerValue];
        ...
        QBUUser *user = [QMCore.instance.usersService.usersMemoryStorage userWithID:uID.integerValue];
        if (!user) {
            user = [QBUUser user];
...
- (NSIndexPath *)indexPathAtUserID:(NSNumber *)userID {
    ...
    QBUUser *user = [QMCore.instance.usersService.usersMemoryStorage userWithID:userID.unsignedIntegerValue];
